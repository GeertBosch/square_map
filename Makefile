...
quickplot: quickbench
	@if [ -f $(BUILD_DIR)/quickbench_results.json ]; then \
		source .venv/bin/activate && PLOT_FILE=$$(python3 quickplot.py $(BUILD_DIR)/quickbench_results.json); \
		if [ -n "$$PLOT_FILE" ]; then \
		...
		echo "No quickbench results found. Run 'make quickbench' first."; \
	fi
...
help:
	...
	@echo "  quickbench - Build and run quick benchmarks (square_map only, Random key order)"
